...
SELECT r.id, p.id
FROM roles r
JOIN permissions p ON p.code IN ('projects:view', 'projects:edit')
WHERE r.name = 'manager';

INSERT OR IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r
JOIN permissions p ON p.code IN ('projects:view')
WHERE r.name = 'viewer';
...
SELECT 'Admin User', 'admin@planner.local', '9a4aabf0e5cf71cae2cea646613ce7e2a5919fa758e56819704be25a3a2c1f0b', r.id
FROM roles r
WHERE r.name = 'admin';
